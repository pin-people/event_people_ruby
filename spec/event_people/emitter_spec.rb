describe EventPeople::Emitter do
  let(:event) { EventPeople::Event.new(name, body) }

  describe '.trigger' do
    before do
      allow(EventPeople::Config.broker).to receive(:produce)
    end
    ...
    context 'when event is valid' do
      ...
      it 'produces the event on the broker' do
        expect(EventPeople::Config.broker).to receive(:produce).with([event])
    ...
    context 'when receives multiple events as parameters' do
      let(:event1) { EventPeople::Event.new('name1', { amount: 200, name: 'Paul' }) }
      let(:event2) { EventPeople::Event.new('name2', { amount: 400, name: 'Ringo' }) }
      let(:body) { { amount: 300, name: 'George' } }
      let(:name) { 'name' }
      ...
      it 'produces the events on the broker' do
        expect(EventPeople::Config.broker).to receive(:produce).with([event, event1, event2])
    ...
    context 'when receives an array of events' do
      ...
      it 'produces the events on the broker' do
        expect(EventPeople::Config.broker).to receive(:produce).with(events)
    ...
    context 'when event body is not present' do
      ...
      it 'does not produce the event on the broker' do
        expect(EventPeople::Config.broker).not_to receive(:produce)
