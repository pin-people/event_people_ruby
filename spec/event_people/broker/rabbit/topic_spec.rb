describe EventPeople::Broker::Rabbit::Topic do
  let(:instance) { described_class.new(connection) }
  let(:connection) { instance_double(Bunny::Channel) }
  ...
  before do
    ...
    allow(connection).to receive(:topic).with(EventPeople::Config::TOPIC, topic_options).and_return(topic)
  end
  ...
  describe '#topic' do
    ...
    context 'when topic is not set up' do
      it 'creates a topic' do
        expect(connection).to receive(:topic).with(EventPeople::Config::TOPIC, topic_options)
  ...
  describe '.produce' do
    ...
    context 'with non defaut schemaVersion' do
      ...
      let(:event) { EventPeople::Event.new(routing_key, body, schemaVersion) }
