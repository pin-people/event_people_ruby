describe EventPeople::Broker::Rabbit do
  ...
  describe '#produce' do
    ...
    context 'when event is a single object' do
      ...
      it 'calls produce on Rabbit Topic class only once' do
        expect(EventPeople::Broker::Rabbit::Topic).to receive(:produce).once

        subject
      end

      it 'calls produce on Rabbit Topic class' do
        expect(EventPeople::Broker::Rabbit::Topic).to receive(:produce).with(channel, events.first)
    ...
    context 'when event is an array of events' do
      ...
      it 'calls produce on Rabbit Topic class only once' do
        expect(EventPeople::Broker::Rabbit::Topic).to receive(:produce).exactly(3).times
      ...
      it 'calls produce on Rabbit Topic class' do
        events.each do |event|
          expect(EventPeople::Broker::Rabbit::Topic).to receive(:produce).with(channel, event)
        end
